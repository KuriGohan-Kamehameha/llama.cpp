...
// =========================================================================
// Q4_1 — 4-bit legacy quant with per-block min (raw block layout).
//
// block_q4_1 = 20 bytes:
//   dm = (d, m) ggml_half2          (4 B)
//   qs[QK4_1/2] = qs[16]             (16 B)  — nibbles, 2 weights per byte
//
// Weight decode (same nibble layout as Q4_0):
//   For block b, output position l in 0..31:
//     k = l & 0xF
//     w[l] = (qs[k] >> ((l < 16) ? 0 : 4)) & 0x0F     unsigned [0, 15]
//   i.e. qs[k] low nibble  -> weight at output l = k        (k = 0..15)
//        qs[k] high nibble -> weight at output l = k + 16
//
// Per-block formula (mirrors vec_dot_q4_1_q8_1_impl):
//   contrib[b] = d_b * dy_b * sumi_b + m_b * sy_b
// where sumi_b = sum_{l=0..31}(w[l] * u_l) (int dot, unsigned weights),
//       sy_b = sum_{l=0..31}(d_y * u_l) precomputed in q8_1.ds[1].
//
// Q4_1 each block is exactly 32 weights = one q8_1 sub-block. We process
// 8 blocks per super-iteration so the 256-byte y_q_shared activation
// load aligns with 8 q8_1 sub-blocks (matches Q4_0 / Q8_0).
//
// Pattern: Q4_0's group-of-8-blocks structure (same nibble decode,
// same pair-of-blocks multiplication) with Q4_K-style additive
// min term replacing the -8*sy offset.
// =========================================================================

template <int VEC_W>
static void esimd_q4_1_kernel(
        const void * __restrict__ vx,
        const void * __restrict__ vy,
        float * __restrict__ dst,
        const int ncols, const int nrows,
    ...
    constexpr int Q41_BLOCKS_PER_GROUP = 8;     // 8 blocks * 32 weights = 256
    constexpr int BLOCK_BYTES          = 2 * sizeof(ggml_half) + (QK4_1 / 2);  // 20
    constexpr int OFF_QS               = 2 * sizeof(ggml_half);                // 4
    constexpr int GROUP_BYTES          = Q41_BLOCKS_PER_GROUP * BLOCK_BYTES;    // 160
    constexpr int GROUP_Y_QS_BYTES     = Q41_BLOCKS_PER_GROUP * QK4_1;          // 256

    static_assert(BLOCK_BYTES == 20, "block_q4_1 size assumption");

    const int n_blocks_per_row = ncols / QK4_1;
    const int n_groups         = n_blocks_per_row / Q41_BLOCKS_PER_GROUP;
    ...
    for (int g = 0; g < n_groups; ++g) {
        ...
        // Per-q8_1-sub-block dy and sy (one (d,s) pair per block — Q4_1
        // blocks are 1:1 with q8_1 sub-blocks).
        const uint16_t * y_ds_u16 = reinterpret_cast<const uint16_t *>(
            y_ds_u8 + g * Q41_BLOCKS_PER_GROUP * sizeof(sycl::half2));
        esimd::simd<float, 8> dy_v, sy_v;
        #pragma unroll
        for (int i = 0; i < 8; ++i) {
            dy_v[i] = static_cast<float>(sycl::bit_cast<sycl::half>(y_ds_u16[i * 2]));
        ...
        for (int r = 0; r < ROWS_PER_THREAD; ++r) {
            ...
            const auto * group_base =
                x_base + (row * n_blocks_per_row + g * Q41_BLOCKS_PER_GROUP) * BLOCK_BYTES;

            // Per-block d and m (8 of each). Q4_1's dm is at byte 0 of each
            // 20-byte block — strided uint16 reads.
            esimd::simd<float, 8> d_v, m_v;
            #pragma unroll
            for (int i = 0; i < 8; ++i) {
                const uint16_t * dm_u16 = reinterpret_cast<const uint16_t *>(
                    group_base + i * BLOCK_BYTES);
                d_v[i] = static_cast<float>(sycl::bit_cast<sycl::half>(dm_u16[0]));
                m_v[i] = static_cast<float>(sycl::bit_cast<sycl::half>(dm_u16[1]));
            }

            // sumi per block: int4 nibble dot int8 activations.
            // Process 2 blocks per pair (64 weights) to bound GRF pressure —
            // matches Q4_0's structure exactly. Each pair loads 2 separate
            // 16-byte qs slices (since blocks are not contiguous in raw
            // layout, unlike Q4_0's reorder).
            esimd::simd<float, 8> sumi_v;
            #pragma unroll
            for (int sub = 0; sub < 8; sub += 2) {
                // 16 bytes of qs per block.
                esimd::simd<uint8_t, 16> qs0;
                qs0.copy_from(group_base +  sub      * BLOCK_BYTES + OFF_QS);
                esimd::simd<uint8_t, 16> qs1;
                qs1.copy_from(group_base + (sub + 1) * BLOCK_BYTES + OFF_QS);

                // Block `sub`: byte k -> weight k (low) and weight k+16 (high).
                esimd::simd<int8_t, 16> w0_lo = qs0 & uint8_t(0x0F);
                esimd::simd<int8_t, 16> w0_hi = (qs0 >> 4) & uint8_t(0x0F);
                // Block `sub+1`: same.
                esimd::simd<int8_t, 16> w1_lo = qs1 & uint8_t(0x0F);
                esimd::simd<int8_t, 16> w1_hi = (qs1 >> 4) & uint8_t(0x0F);
            ...
            // Per-block contrib: d * dy * sumi + m * sy
...
void mul_mat_vec_q4_1_q8_1_sycl_esimd(
        const void * vx, const void * vy, float * dst,
        const int ncols, const int nrows,
        dpct::queue_ptr stream) {
    GGML_ASSERT(ncols % (QK4_1 * 8) == 0);  // need multiple of 8 blocks per row
    ...
    stream->submit([&](sycl::handler & cgh) {
        cgh.parallel_for(sycl::nd_range<1>(global_size, workgroup_size),
            [=](sycl::nd_item<1> nd_item) SYCL_ESIMD_KERNEL {
                esimd_q4_1_kernel<32>(vx, vy, dst, ncols, nrows, nd_item);
            });
    });
}
...
// =========================================================================
// Q5_0 — 5-bit legacy quant with signed offset (raw block layout).
//
// block_q5_0 = 22 bytes:
//   d   (ggml_half)              (2 B)
//   qh[4]                         (4 B)   — 5th bit of each weight (32 bits)
//   qs[QK5_0/2] = qs[16]          (16 B)  — low 4 bits, nibble pair per byte
//
// Weight decode (5-bit unsigned, then -16 offset for signed [-16, 15]):
//   For block b, output position l in 0..31:
//     k        = l & 0xF
//     low4(l)  = (qs[k] >> ((l < 16) ? 0 : 4)) & 0x0F
//     hbit(l)  = (qh[l/8] >> (l%8)) & 0x1     -- qh is a 32-bit bitmap
//     w_u5(l)  = low4(l) | (hbit(l) << 4)     -- unsigned [0, 31]
//
// Per-block formula (mirrors vec_dot_q5_0_q8_1_impl):
//   contrib[b] = d_b * (sumi_b * dy_b - 16 * sy_b)
// where sumi_b = sum_{l=0..31}(w_u5(l) * u_l).
// The -16 offset converts unsigned [0,31] -> signed [-16,15]; the same
// shape as Q4_0's -8*sy*d but with a different constant.
//
// Pattern: Q4_0's group-of-8-blocks structure + Q5_K-style qh bit
// extract (Q5_0 differs in that qh is just 4 bytes covering 32 weights
// directly, vs Q5_K's 32-byte qh with per-sub-block bit selection).
// =========================================================================

template <int VEC_W>
static void esimd_q5_0_kernel(
        const void * __restrict__ vx,
        const void * __restrict__ vy,
        float * __restrict__ dst,
        const int ncols, const int nrows,
    ...
    constexpr int Q50_BLOCKS_PER_GROUP = 8;     // 8 blocks * 32 weights = 256
    constexpr int BLOCK_BYTES          = sizeof(ggml_half) + 4 + (QK5_0 / 2);   // 22
    constexpr int OFF_D                = 0;
    constexpr int OFF_QH               = sizeof(ggml_half);                     // 2
    constexpr int OFF_QS               = OFF_QH + 4;                             // 6
    constexpr int GROUP_Y_QS_BYTES     = Q50_BLOCKS_PER_GROUP * QK5_0;          // 256
    static_assert(BLOCK_BYTES == 22, "block_q5_0 size assumption");

    const int n_blocks_per_row = ncols / QK5_0;
    const int n_groups         = n_blocks_per_row / Q50_BLOCKS_PER_GROUP;
    ...
    for (int g = 0; g < n_groups; ++g) {
        ...
        // dy and sy per q8_1 sub-block — Q5_0 blocks are 1:1 with q8_1 sub-blocks.
        const uint16_t * y_ds_u16 = reinterpret_cast<const uint16_t *>(
            y_ds_u8 + g * Q50_BLOCKS_PER_GROUP * sizeof(sycl::half2));
        esimd::simd<float, 8> dy_v, sy_v;
        #pragma unroll
        for (int i = 0; i < 8; ++i) {
            dy_v[i] = static_cast<float>(sycl::bit_cast<sycl::half>(y_ds_u16[i * 2]));
        ...
        for (int r = 0; r < ROWS_PER_THREAD; ++r) {
            ...
            const auto * group_base =
                x_base + (row * n_blocks_per_row + g * Q50_BLOCKS_PER_GROUP) * BLOCK_BYTES;

            // Per-block d (8 of them) via strided uint16 reads at offset 0.
            esimd::simd<float, 8> d_v;
            #pragma unroll
            for (int i = 0; i < 8; ++i) {
                const uint16_t * d_u16 = reinterpret_cast<const uint16_t *>(
                    group_base + i * BLOCK_BYTES + OFF_D);
                d_v[i] = static_cast<float>(sycl::bit_cast<sycl::half>(*d_u16));
            }

            // Per-block sumi via 5-bit unsigned dot product.
            esimd::simd<float, 8> sumi_v;
            #pragma unroll
            for (int b = 0; b < 8; ++b) {
                const auto * blk = group_base + b * BLOCK_BYTES;

                // qh[4]: 32 bits, one per weight.
                esimd::simd<uint8_t, 4> qh_v;
                qh_v.copy_from(blk + OFF_QH);

                // qs[16]: nibble-packed low 4 bits.
                esimd::simd<uint8_t, 16> qs_v;
                qs_v.copy_from(blk + OFF_QS);

                // Low 4 bits per output position (32 unsigned values [0,15]).
                esimd::simd<int8_t, 32> low4;
                low4.template select<16, 1>(0)  = qs_v & uint8_t(0x0F);
                low4.template select<16, 1>(16) = (qs_v >> 4) & uint8_t(0x0F);

                // High bit per output position from qh bitmap.
                // hbit[8*byte + bit] = (qh[byte] >> bit) & 1
                esimd::simd<int8_t, 32> hbit;
                #pragma unroll
                for (int by = 0; by < 4; ++by) {
                    const uint8_t qh_byte = qh_v[by];
                    #pragma unroll
                    for (int bit = 0; bit < 8; ++bit) {
                        hbit[8 * by + bit] =
                            static_cast<int8_t>((qh_byte >> bit) & 0x01);
                    }
                }

                // Unsigned 5-bit weights [0, 31] = low4 | (hbit << 4).
                esimd::simd<int8_t, 32> w_chunk = low4 | (hbit << 4);

                // Activation for this q8_1 sub-block.
                esimd::simd<int8_t, 32> y_chunk =
                    y_q_shared.template select<32, 1>(b * 32);

                esimd::simd<int16_t, 32> prod = w_chunk * y_chunk;
                sumi_v[b] = static_cast<float>(esimd::reduce<int>(prod, std::plus<>{}));
            }

            // Per-block contrib: d * (sumi * dy - 16 * sy)
            // (the -16 offset converts unsigned 5-bit [0,31] -> signed [-16,15];
            //  same shape as Q4_0's d * (sumi * dy - 8 * sy) with a different
            //  constant.)
            const esimd::simd<float, 8> contrib =
                d_v * (sumi_v * dy_v - 16.0f * sy_v);
            acc[r] += esimd::reduce<float>(contrib, std::plus<>{});
        }
    }
...
void mul_mat_vec_q5_0_q8_1_sycl_esimd(
        const void * vx, const void * vy, float * dst,
        const int ncols, const int nrows,
        dpct::queue_ptr stream) {
    GGML_ASSERT(ncols % (QK5_0 * 8) == 0);  // need multiple of 8 blocks per row
    ...
    stream->submit([&](sycl::handler & cgh) {
        cgh.parallel_for(sycl::nd_range<1>(global_size, workgroup_size),
            [=](sycl::nd_item<1> nd_item) SYCL_ESIMD_KERNEL {
                esimd_q5_0_kernel<32>(vx, vy, dst, ncols, nrows, nd_item);
            });
    });
}
...
// =========================================================================
// Q5_1 — 5-bit legacy quant with per-block min (raw block layout).
//
// block_q5_1 = 24 bytes:
//   dm  = (d, m) ggml_half2        (4 B)
//   qh[4]                          (4 B)   — 5th bit of each weight (32 bits)
//   qs[QK5_1/2] = qs[16]           (16 B)  — low 4 bits, nibble pair per byte
//
// Weight decode (5-bit unsigned, NO offset — the min is applied via m):
//   For block b, output position l in 0..31:
//     k        = l & 0xF
//     low4(l)  = (qs[k] >> ((l < 16) ? 0 : 4)) & 0x0F
//     hbit(l)  = (qh[l/8] >> (l%8)) & 0x1     -- qh is a 32-bit bitmap (same as Q5_0)
//     w_u5(l)  = low4(l) | (hbit(l) << 4)     -- unsigned [0, 31]
//
// Per-block formula (mirrors vec_dot_q5_1_q8_1_impl):
//   contrib[b] = d_b * dy_b * sumi_b + m_b * sy_b
// where sumi_b = sum_{l=0..31}(w_u5(l) * u_l).
// Q5_1 vs Q5_0: weights stay unsigned (no -16 subtract); the offset is
// absorbed into the per-block m and applied via m*sy. Same relationship
// as Q4_1 vs Q4_0.
//
// Pattern: Q5_0 kernel structure + Q4_1's additive min term.
// =========================================================================

template <int VEC_W>
static void esimd_q5_1_kernel(
        const void * __restrict__ vx,
        const void * __restrict__ vy,
        float * __restrict__ dst,
        const int ncols, const int nrows,
    ...
    constexpr int Q51_BLOCKS_PER_GROUP = 8;     // 8 blocks * 32 weights = 256
    constexpr int BLOCK_BYTES          = 2 * sizeof(ggml_half) + 4 + (QK5_1 / 2);  // 24
    constexpr int OFF_DM               = 0;
    constexpr int OFF_QH               = 2 * sizeof(ggml_half);                    // 4
    constexpr int OFF_QS               = OFF_QH + 4;                                // 8
    constexpr int GROUP_Y_QS_BYTES     = Q51_BLOCKS_PER_GROUP * QK5_1;              // 256
    static_assert(BLOCK_BYTES == 24, "block_q5_1 size assumption");

    const int n_blocks_per_row = ncols / QK5_1;
    const int n_groups         = n_blocks_per_row / Q51_BLOCKS_PER_GROUP;
    ...
    for (int g = 0; g < n_groups; ++g) {
        ...
        // dy and sy per q8_1 sub-block — Q5_1 blocks are 1:1 with q8_1 sub-blocks.
        const uint16_t * y_ds_u16 = reinterpret_cast<const uint16_t *>(
            y_ds_u8 + g * Q51_BLOCKS_PER_GROUP * sizeof(sycl::half2));
        esimd::simd<float, 8> dy_v, sy_v;
        #pragma unroll
        for (int i = 0; i < 8; ++i) {
            dy_v[i] = static_cast<float>(sycl::bit_cast<sycl::half>(y_ds_u16[i * 2]));
        ...
        for (int r = 0; r < ROWS_PER_THREAD; ++r) {
            ...
            const auto * group_base =
                x_base + (row * n_blocks_per_row + g * Q51_BLOCKS_PER_GROUP) * BLOCK_BYTES;

            // Per-block (d, m) via strided uint16 reads at offset 0.
            esimd::simd<float, 8> d_v, m_v;
            #pragma unroll
            for (int i = 0; i < 8; ++i) {
                const uint16_t * dm_u16 = reinterpret_cast<const uint16_t *>(
                    group_base + i * BLOCK_BYTES + OFF_DM);
                d_v[i] = static_cast<float>(sycl::bit_cast<sycl::half>(dm_u16[0]));
                m_v[i] = static_cast<float>(sycl::bit_cast<sycl::half>(dm_u16[1]));
            }

            // Per-block sumi via 5-bit unsigned dot product (same decode as Q5_0).
            esimd::simd<float, 8> sumi_v;
            #pragma unroll
            for (int b = 0; b < 8; ++b) {
                const auto * blk = group_base + b * BLOCK_BYTES;
            ...
            // Per-block contrib: d * dy * sumi + m * sy
            // (Q5_1 weights stay unsigned; the offset is absorbed into m,
            //  applied additively. Same relationship as Q4_1 vs Q4_0.)
            const esimd::simd<float, 8> contrib =
                d_v * dy_v * sumi_v + m_v * sy_v;
            acc[r] += esimd::reduce<float>(contrib, std::plus<>{});
        }
    }
...
void mul_mat_vec_q5_1_q8_1_sycl_esimd(
        const void * vx, const void * vy, float * dst,
        const int ncols, const int nrows,
        dpct::queue_ptr stream) {
    GGML_ASSERT(ncols % (QK5_1 * 8) == 0);  // need multiple of 8 blocks per row
    ...
    stream->submit([&](sycl::handler & cgh) {
        cgh.parallel_for(sycl::nd_range<1>(global_size, workgroup_size),
            [=](sycl::nd_item<1> nd_item) SYCL_ESIMD_KERNEL {
                esimd_q5_1_kernel<32>(vx, vy, dst, ncols, nrows, nd_item);
            });
    });
}
